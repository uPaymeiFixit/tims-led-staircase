...
#define FRAMES_PER_SECOND 20  // Maximum framerate
#define MAX_SONAR_DISTANCE 35 // Don't detect past this (inches)
#define NUM_LEDS 42           // Number of steps
#define NUM_STEPS 13
#define LEDS_PER_STEP 2
...
void setup()
{
  ...
  FastLED.addLeds<LED_CHIP, LED_DATA_PIN, COLOR_ORDER>(leds, NUM_LEDS).setCorrection(TypicalLEDStrip);
...
CRGB animation_stack_leds[ANIMATION_STACK_SIZE_MAX][NUM_LEDS];
...
const unsigned int ANIMATION_RUNTIME = 20000 + NUM_LEDS * 1000l;
...
void animationLoop()
{
  ...
  {
    // Call the stack item
    animate(animation_stack_start_times[i], animation_stack_starting_point[i], animation_stack_leds[i]);
  }
}

// Animate a single frame of the LEDs
void animate(unsigned long start_time, bool starting_point, CRGB vleds[])
{
  ...
  for (int i = 0; i < NUM_LEDS; i++)
  {
    ...
    const int led_index = starting_point ? i : NUM_LEDS - i;
    vleds[led_index].r = brightness;
    vleds[led_index].g = brightness;
    vleds[led_index].b = brightness;
  }
}
...
void popStack()
{
  for (int i = 0; i < animation_stack_size - 1; i++)
  {
    ...
    *animation_stack_leds[i] = &animation_stack_leds[i + 1];
  }
  animation_stack_size--;
...
void displayLeds()
{
  for (int i = 0; i < NUM_LEDS; i++)
  {
    ...
    for (int j = 0; j < animation_stack_size; j++)
    {
      const unsigned int sum_r = leds[i].r + animation_stack_leds[j][i].r;
      leds[i].r = sum_r > 255 ? 255 : sum_r;
      const unsigned int sum_g = leds[i].g + animation_stack_leds[j][i].g;
      leds[i].g = sum_g > 255 ? 255 : sum_g;
      const unsigned int sum_b = leds[i].b + animation_stack_leds[j][i].b;
